...
const RestaurantOrders = () => {
  ...
  const fetchOrders = async () => {
    try {
      ...
      // Extract unique restaurant names and fix type error
      const uniqueRestaurants = Array.from(
        new Set(data.map((order: Order) => order.restaurant))
      ) as string[];
  ...
  const handleMarkAsPrepared = async (orderId: number) => {
    try {
      await axios.put(`http://localhost:8080/restaurant/orders/${orderId}/prepare`);
      ...
      <div className="space-y-10 sm:space-y-12">
        <div className="flex justify-between items-center">
          <div className="flex items-center space-x-4">
            ...
            <h2 className="text-2xl sm:text-3xl font-semibold">Restaurant Orders</h2>
          </div>
          <div className="flex items-center space-x-4">
            <select
